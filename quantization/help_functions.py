...
from quantization.quant_functions import ScalingFunction
...
def get_huffman_encoding_mean_bit_length(model_param_iter, quantization_functions, type_quantization='uniform',
    ...
    if not isinstance(quantization_functions, dict):
        quantization_functions = {'all':quantization_functions}

    single_quant_fun = len(quantization_functions) == 1
    total_length = 0
    frequency = defaultdict(int)
    tol = 1e-5
    for idx, (name,param) in enumerate(model_param_iter):
        param = param.clone()
        ...
        if single_quant_fun:
            quant_fun = quantization_functions['all']
        else:
            quant_fun = quantization_functions[name]
        ...
        if type_quantization == 'uniform':
            ...
            '''
                ...
                raise ValueError('param s is wrong {}'.format(s))
            '''

            #if not isinstance(s_,int):
            #    s_=s_[0]
            #print(s_)
            quant_points = [x / (s_-1) for x in range(s_)]
...
def check_right_bits(tensor_iterator, num_quant_points, bucket_size):
    ...
    scaling_function = ScalingFunction('linear', False, False, bucket_size=bucket_size)
